<?php
    ...
    function cambiarPass() {
        ...
        if ($verificar) { //REALIZAMOS LA CONSULTA
            ...
            $clavehash = password_hash($nuevapass, PASSWORD_DEFAULT);
            $consultanewpass = "UPDATE USUARIO SET clave='$clavehash' WHERE username='$usuario'";  
            $stm2 = $con->query($consultanewpass);
            header("location: ../inicio?error=false");

        }    else {

            header("location: ../inicio?error=true");
    ...
    function isPantalla(){
        ...
        if($resultado){
            ...
            // header("location: pantalla.php?mac='$mac_cliente'");
        }
    }
    ...
    function insertarPantalla(){
        ...
        try{
            ...
            header("Location: ../login?screen=true");
            // header("location: ../inicio");
    ...
    //ACTIVA LA FUNCIÓN DE UPDATE PANTALLA SI SE ENVIA POR POST ALGO DESDE EL BOTÓN 'pantaButtonEdit'
    if(isset($_POST['pantaButtonEdit'])) {
        upadtePantalla(); 
    }

    function upadtePantalla(){
        //CONECTAMOS A LA BD
        $conexion = conexion();
        ...
        //echo $mac_pantalla. " ".$ubicacion. " ".$nombre;
        $sqlUpdatePantalla = "UPDATE PANTALLA SET ubicacion = '$ubicacion' , nombre = '$nombre' WHERE mac_pantalla = '$mac_pantalla'";

        //echo $sqlUpdatePantalla;
        $consulta=$conexion->prepare($sqlUpdatePantalla);
        $consulta->execute();
    ...
    function insertarUsuario(){
        ...
        if (noRepetidos($_POST['username'], $_POST['email'])){
            ...
            //REDIRECCIONAMOS
            header("Location: ../inicio?user=true");
        }else{
            $conexion = null;
    ...
    //LLAMA A UPDATE USUARIO SI SE PULSO USERBUTTON
    if(isset($_POST['userButtonEdit'])) {
        updateUsuario(); 
    }
    ...
    function updateUsuario(){
        $conexion = conexion();
            ...
            //echo $username. " ".$clave. " ".$nombre." ".$email." ".$dni." ".$id_rol;
            $sqlUpdateUser = "UPDATE USUARIO SET clave = '$clave' , nombre = '$nombre', email = '$email', dni = '$dni', id_rol = '$id_rol' WHERE username = '$username'";

            //echo $sqlUpdateUser;
            $consulta=$conexion->prepare($sqlUpdateUser);
            $consulta->execute();
            $conexion = null; //CERRAMOS LA CONEXION DE LA BD
    ...
    function insertarPublicacion(){
        ...
        try{
            ...
            //REGRESAMOS A LA PÁGINA INDEX (PENDIENTE SACAR UN MENSAJE DE APROBACIÓN)
            header("location: ../inicio?publi=true");
    ...
    //ACTIVA LA FUNCIÓN DE UPDATE PUBLICACIÓN SI SE ENVIA POR POST ALGO DESDE EL BOTÓN 'publiButton'
    if(isset($_POST['publiButtonEdit'])) {
        updatePublicacion(); 
    }
    ...
    function updatePublicacion(){
        ...
        // echo $id_publicacion. " ".$titulo." ".$mensaje." ".$fechaInicio. " ".$fechaInicio;
